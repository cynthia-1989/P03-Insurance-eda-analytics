...
# P02 Insurance SQL — Data Extractor
# ================================================================
...
class DataExtractor:
    """
    Runs the insurance production extraction query and saves raw-data.csv.
    ...
    def extract(self) -> "DataExtractor":
        """Run the insurance extraction SQL query using the real database only."""
        logger.info(f"[EXTRACT] Starting insurance extraction — industry: {self.industry}")
        ...
        if len(self.raw_df) == 0:
            raise RuntimeError(
                "SQL returned 0 rows. Check your joins or confirm the insurance tables have data."
            )
    ...
    def save(self) -> "DataExtractor":
        """Save extracted insurance data to data/raw/raw-data.csv."""
    ...
    def report(self) -> None:
        ...
        print()
        print("=" * 70)
        print("  P02 INSURANCE SQL — EXTRACTION COMPLETE")
        print("=" * 70)
