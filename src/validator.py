...
#
# CONTEXT: We just ran Module 03 and extracted insurance data from
# the insurance database into raw-data.csv.
# Before we clean anything, we need to know WHAT is wrong.
# That is exactly what this file does.
#
# THE ANALOGY:
# Imagine you work in an insurance claims department.
# Before a claim can be approved, the system runs checks first.
# The checks do not approve the claim — they just report what is wrong.
# A DataValidator is the quality check for your insurance data.
...
#   4. check_numeric_ranges() → are there impossible values (negative claim amount)?
...
class DataValidator:
    ...
    def __init__(self, df: pd.DataFrame):
        ...
        # Example: {"severity": "WARNING", "column": "amount_claimed", "message": "23 nulls (1.9%)"}
        self.issues = []
    ...
    def check_nulls(self) -> "DataValidator":
        ...
        NULL values are dangerous because:
          - amount_claimed.mean() on a column with NaN returns NaN
        ...
        - Optional claim fields: a customer record where a phone number was not filled.
    ...
    def check_duplicates(self) -> "DataValidator":
        ...
        ──────────────────────────────────────────
        1. SQL JOIN ERRORS
           A JOIN between claims and policies where the join key is not unique
           creates multiple copies of each claim row — one per policy match.
        ...
        ────────────────────────────────────
        If one insurance claim appears 3 times:
          - Your average claim amount calculation is biased upward
          - Your ML model sees the same claim 3× — it overfits to that pattern
          - Your claims report says 1,203 claims when there are 1,200
        """
    ...
    def check_numeric_ranges(self) -> "DataValidator":
        ...
        ────────────────────────────────
        amount_claimed   = -5000   → Insurance claim amounts should not normally be negative
        customer_age     = -3      → A customer's age cannot be negative
        risk_score       = 150     → Risk score should normally be between 0 and 100
        deductible       = -20     → Deductible cannot normally be negative
        ...
        "Difference" columns legitimately go negative:
          claim_vs_policy_avg = -500 means this claim is below the policy average
          approved_gap = -200 means the approved amount is lower than the claimed amount
          premium_adjustment = -100 means the premium was reduced by 100
        ...
        for col in self.df.select_dtypes(include=["number"]).columns:
            ...
            if col == "risk_score":
                invalid_risk = int(((self.df[col] < 0) | (self.df[col] > 100)).sum())
                ...
                if invalid_risk > 0:
                    self._add_issue(
                        ...
                        column=col,
                        message=(
                            f"{invalid_risk} invalid risk score values. "
                            "Risk score should be between 0 and 100."
                        )
                    )

            if col == "fraud_probability":
                invalid_probability = int(((self.df[col] < 0) | (self.df[col] > 1)).sum())
                ...
                if invalid_probability > 0:
                    self._add_issue(
                        ...
                        column=col,
                        message=(
                            f"{invalid_probability} invalid fraud probability values. "
                            "Fraud probability should be between 0 and 1."
                        )
                    )

            if col == "commission_rate":
                invalid_commission = int(((self.df[col] < 0) | (self.df[col] > 100)).sum())
                ...
                if invalid_commission > 0:
                    self._add_issue(
                        ...
                        column=col,
                        message=(
                            f"{invalid_commission} invalid commission rate values. "
    ...
    def compute_stats(self) -> "DataValidator":
        ...
        A dictionary (dict) maps keys to values:
            my_dict = {"name": "Kwame", "amount_claimed": 92000}
            my_dict["amount_claimed"]  → 92000
...
if __name__ == "__main__":
    ...
    clean_df = pd.DataFrame({
        "claim_id": [1, 2, 3, 4, 5],
        "policy_type": ["Auto", "Health", "Property", "Travel", "Life"],
        "claim_type": ["Collision", "Medical", "Fire", "Trip Cancellation", "Life"],
        "amount_claimed": [1200, 5000, 3200, 1500, 8000],
        "amount_approved": [1000, 4500, 3000, 1300, 7600],
        "risk_score": [45, 70, 55, 20, 80],
        "fraud_probability": [0.10, 0.80, 0.20, 0.05, 0.90],
    })
    ...
    dirty_df.loc[0, "amount_claimed"] = None   # introduce one null
