...
#   Which specific insurance claims have amounts that are statistically unusual?
#   Which policy holders have risk scores that no normal customer should produce?
#
# THE BUSINESS QUESTION:
#   "We know the average claim amount is $18k. But are there any claims whose
#    amounts are so far from normal that we should investigate them?
#    Either they are fraudulent, incorrectly entered, or operationally risky."
#
# THE ANALOGY:
#   Imagine plotting all insurance claim amounts on a number line.
...
class AnomalyDetector:
    ...
    def _detect_zscore(self, col: str) -> pd.Series:
        ...
        LIMITATION:
        ───────────
        If the data is highly skewed (e.g. claim amount data), the mean and std
    ...
    def save_anomalies(self) -> "AnomalyDetector":
        ...
        Anomaly rows need to be reviewed by a human:
          - Is this a fraudulent claim? → investigate immediately
          - Is this a data entry error? → fix in the source system
          - Is this a legitimate high-risk policy? → escalate for review

        The CSV allows the insurance team to review without needing Python.
