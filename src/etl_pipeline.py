...
from config import (
    ...
    PROCESSED_DATA_PATH,     # where to save processed-data.csv
...
class ETLPipeline:
    ...
    def __init__(self):
        ...
        logger.info(
            ...
            f"output: {PROCESSED_DATA_PATH.name}"
        )
    ...
    def transform(self) -> "ETLPipeline":
        ...
        (
            ...
            .add_derived_columns()   # step 4: add insurance analytics columns
    ...
    def load(self) -> "ETLPipeline":
        ...
        logger.info(f"[LOAD] Saving {len(processed_df):,} rows to: {PROCESSED_DATA_PATH}")
        ...
        file_size_kb = PROCESSED_DATA_PATH.stat().st_size / 1024   # bytes → KB

        self._status = "loaded"
        self._log(
            f"Saved {len(processed_df):,} rows to "
            f"{PROCESSED_DATA_PATH.name} ({file_size_kb:.1f} KB)"
        )
    ...
    def report(self) -> None:
        ...
        print()
        print("═" * 62)
        print(f"  MODULE 05 — INSURANCE ETL PIPELINE COMPLETE")
        print("═" * 62)
        ...
        file_size_kb = PROCESSED_DATA_PATH.stat().st_size / 1024
        print(f"    File size:       {file_size_kb:.1f} KB")

        # ── What comes next ────────────────────────────────────────────
        print()
        print("  NEXT MODULES THAT USE THIS OUTPUT:")
        print(f"    → Module 06 EDA:    reads {PROCESSED_DATA_PATH.name}")
        print(f"    → Module 09 ML:     trains on {PROCESSED_DATA_PATH.name}")
