# ================================================================
# tests/test_sql.py — Unit Tests for Insurance Module
# ================================================================
...
def test_query_runner_returns_dataframe():
    ...
    # Pass a trivially simple query — works even if insurance table does not exist
...
def test_extractor_synthetic_data_has_required_columns():
    """Synthetic fallback insurance data must contain all required columns."""
    ...
    for col in required:
        assert col in raw.columns, \
            f"Synthetic data missing column: {col}."

    print("  PASS: test_extractor_synthetic_data_has_required_columns")


def test_extractor_synthetic_data_has_quality_issues():
    """Synthetic insurance data must contain intentional data quality issues."""

    raw = DataExtractor._synthetic_raw_data(300)

    # Should have some NULL claim amounts
    null_claims = raw["amount_claimed"].isna().sum()

    assert null_claims > 0, \
        "Synthetic raw data should have some NULL claim amounts (data quality issues)"

    # Should have some abnormal risk_score entries
    weird_scores = (raw["risk_score"] > 100).sum()

    assert weird_scores > 0, \
        "Synthetic raw data should have some abnormal risk_score entries"
...
def test_extractor_save_creates_csv(tmp_path):
    ...
    assert cfg.RAW_DATA_PATH.exists(), \
        "save() should create raw-data.csv"

    reloaded = pd.read_csv(cfg.RAW_DATA_PATH)

    assert len(reloaded) == 50, \
        "Saved CSV should have correct row count"
...
if __name__ == "__main__":
    ...
    print()
    print("=" * 60)
    print("  INSURANCE MODULE — SQL UNIT TESTS")
    print("=" * 60)
    print()
