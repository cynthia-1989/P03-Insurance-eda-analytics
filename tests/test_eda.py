...
# WHY TEST EDA CODE?
# ──────────────────
# EDA results feed into ML feature selection, fraud analytics,
# insurance risk analysis, and MLOps monitoring.
# If the EDAEngine computes incorrect statistics,
...
def make_sample_df(rows: int = 50) -> pd.DataFrame:
    ...
    claim_types = ["Auto", "Health", "Property", "Travel", "Life"]
    policy_types = ["Basic", "Premium", "Business", "Family", "Corporate"]

    amount_claimed = np.random.uniform(500, 50000, rows).round(2)
    amount_approved = (
        amount_claimed *
        np.random.uniform(0.6, 1.0, rows)
    ).round(2)

    risk_score = np.random.randint(1, 100, rows)

    return pd.DataFrame({
        "claim_id": range(1, rows + 1),
        "claim_type": [
...
def test_profile_computes_correct_row_count():
    ...
    assert engine.results["profile"]["rows"] == 50, \
        "Profile row count should equal the DataFrame length"
...
def test_profile_null_count_is_zero_for_clean_data():
    ...
    assert engine.results["profile"]["total_nulls"] == 0, \
        "Clean DataFrame should have 0 nulls in profile"
...
def test_group_analysis_produces_results():
    ...
    assert "group_analysis" in engine.results, \
        "results dict should contain 'group_analysis' key"

    assert len(engine.results["group_analysis"]) > 0, \
        "group_analysis should have at least one grouping variable"
...
def test_group_analysis_rank_starts_at_one():
    ...
    assert top_row["rank"] == 1, \
        f"First sorted row should have rank=1, got rank={top_row['rank']}"
...
def test_correlation_only_includes_strong_pairs():
    ...
    if "correlation" in engine.results:
        ...
        for pair in engine.results["correlation"]["strong_pairs"]:
            ...
            assert abs(pair["correlation"]) >= CORRELATION_THRESHOLD, \
                (
                    f"Pair {pair['col_a']}↔{pair['col_b']} "
                    f"has r={pair['correlation']:.3f} "
                    f"which is below threshold "
...
def test_method_chaining_returns_self():
    ...
    assert result is engine, \
        "Method chaining must return the same EDAEngine object (self)"
...
def test_anomaly_detector_flags_obvious_outlier():
    ...
    df.loc[0, "amount_claimed"] = 10_000_000

    detector = AnomalyDetector(df)
    detector.run(columns=["amount_claimed"])

    # Row 0 should appear in confirmed anomalies
    assert 0 in detector.confirmed.index, \
        "Row with obvious outlier amount_claimed should be in confirmed anomalies"
...
def test_anomaly_detector_clean_data_has_few_anomalies():
    ...
    df = pd.DataFrame({
        "amount_claimed": np.random.normal(10000, 1000, 200)
    })
    ...
    assert anomaly_pct < 2.0, \
        f"Normal data should have < 2% anomalies, got {anomaly_pct:.1f}%"
...
def test_anomaly_detector_summary_has_correct_columns():
    ...
    for col in expected_cols:
        assert col in summary.columns, \
            f"Summary DataFrame should have column '{col}'"
...
def test_anomaly_detector_original_data_unchanged():
    ...
    original_amount_0 = df.loc[0, "amount_claimed"]

    detector = AnomalyDetector(df)
    detector.run()

    assert df.loc[0, "amount_claimed"] == original_amount_0, \
...
def test_detector_handles_zero_std_column():
    ...
    df = pd.DataFrame({
        "constant": [100.0] * 20,
        "amount_claimed": np.random.uniform(500, 50000, 20),
    })

    # Should not raise ZeroDivisionError or any other exception
    try:
        detector = AnomalyDetector(df)
        detector.run(columns=["constant", "amount_claimed"])
        ...
        raise AssertionError(
            f"Detector crashed on zero-std column: {e}"
        )
...
if __name__ == "__main__":
    print()
    print("=" * 60)
    print("  MODULE 06 — INSURANCE EDA AND ANOMALY DETECTION TESTS")
    print("=" * 60)
    print()
